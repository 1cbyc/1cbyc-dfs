...
1. i've made sure it all works on a basic level, so now i want to add a file upload functionality to allow clients to send files to the server. that way, when the server gets it, it will store the file and acknowledge receipt. Okay, finally implemented file upload. this works well. next i want to implement file download, file indexing, file replication for fault tolerance and then concurrency to handle multiple clients simultaneously using threads or asyn
